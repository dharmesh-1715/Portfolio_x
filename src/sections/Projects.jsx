...
import { FaGithub, FaExternalLinkAlt, FaMobileAlt, FaRobot } from 'react-icons/fa';
...
const PROJECTS = [
  {
    ...
    demo: '#',
    highlights: [
      { icon: FaMobileAlt, text: 'Role-based Access Control (Firebase Auth)' },
      { icon: FaGithub, text: 'Real-time Task & Project Management' },
    ],
    star: true,
    points: [
  ...
  },
  {
    ...
    demo: '#',
    highlights: [
      { icon: FaRobot, text: 'AI Chatbot via Dialogflow' },
    ],
    star: false,
    points: [
  ...
  },
  {
    ...
    desc: 'An interactive quiz application with real-time score tracking, multiple topics, engaging animations, and sound effects for an immersive learning experience.',
    tech: ['Flutter', 'Dart', 'Firebase'],
    github: 'https://github.com/dharmesh-1715/FLUTTER_PRO_QuizApp',
    demo: '#',
    highlights: [],
    star: false,
    points: [
      'Built with Flutter & Dart for a responsive, user-friendly interface.',
      'Engaging sound effects and animated feedback on answers.',
      'Real-time score tracking with smooth animations.',
      'Multiple quiz topics with Firebase real-time data synchronization.',
    ],
    color: '#8b5cf6',
  },
];
...
function ProjectCard({ project, index }) {
  ...
  return (
    ...
    >
      {/* Top color line */}
      <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl" style={{ background: `linear-gradient(90deg, ${project.color}, transparent)` }} />

      <div className="relative z-10 flex-grow flex flex-col">
        {/* Header */}
        <div className="flex justify-between items-start mb-5">
          <div>
            <p className="font-mono text-sm mb-1" style={{ color: project.color }}>{project.type}</p>
            <h3 className={`font-bold text-slate-100 group-hover:text-white transition-colors ${project.star ? 'text-2xl lg:text-3xl' : 'text-xl'}`}>
              {project.title}
            </h3>
          </div>
          <div className="flex gap-3 flex-shrink-0">
            <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-cyan-400 transition-colors" data-cursor-hover>
              <FaGithub size={20} />
            </a>
            <a href={project.demo} className="text-slate-400 hover:text-cyan-400 transition-colors" data-cursor-hover>
        ...
        {/* Description */}
        <p className="text-slate-300 text-sm leading-relaxed mb-5">{project.desc}</p>

        {/* Bullet points */}
        <ul className="space-y-1.5 mb-5 flex-grow">
          {project.points.map((pt, i) => (
            <li key={i} className="flex gap-2 text-slate-400 text-sm">
              <span style={{ color: project.color }} className="flex-shrink-0 mt-0.5 text-xs">▸</span>
              {pt}
            </li>
          ))}
        </ul>
        ...
        {project.star && project.highlights.length > 0 && (
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            {project.highlights.map((hlt, i) => (
              <div key={i} className="flex items-center gap-3 bg-slate-900/50 border border-slate-700/50 px-4 py-2.5 rounded-lg">
                <hlt.icon style={{ color: project.color }} className="text-lg flex-shrink-0" />
                <span className="text-sm font-medium text-slate-200">{hlt.text}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-16 md:mb-24 flex flex-col items-end text-right">
          ...
          >
            ...
            <span className="section-tag !text-blue-500">Engineering Showcase</span>
          </motion.div>
          <motion.h2
